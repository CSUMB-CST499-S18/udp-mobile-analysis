...
mydatafiltered$wUDPLoss1 = as.character(mydatafiltered$wUDPLoss1)
mydatafiltered$eUDPLoss1 = as.character(mydatafiltered$eUDPLoss1)
mydatafiltered$wUDPLoss1 <- gsub("^[a-z].*", "Inf", mydatafiltered$wUDPLoss1)
mydatafiltered$eUDPLoss1 <- gsub("^[a-z].*", "Inf", mydatafiltered$eUDPLoss1)


#convert char to numeric 
mydatafiltered$wUDPLoss1 <- as.numeric(as.character(mydatafiltered$wUDPLoss1))
mydatafiltered$eUDPLoss1 <- as.numeric(as.character(mydatafiltered$eUDPLoss1))
...
getavgWest<-function(x,sdata){
  ...
  newdata<-sdata[1:y,]
  sumnewdata<-sum(newdata$wUDPLoss1)
  print(sumnewdata)
  avgNewData<-sumnewdata/y
  avgNewData<-round(avgNewData, digits = 2)
  return(avgNewData)
}
getavgEast<-function(x,sdata){
  ...
  newdata<-sdata[1:y,]
  sumnewdata<-sum(newdata$eUDPLoss1)
  avgNewData<-sumnewdata/y
  avgNewData<-round(avgNewData, digits = 2)
  return(avgNewData)
}
...
sprintsorted<-sprint[order(sprint$wUDPLoss1),]
sprintSortedEast<-sprint[order(sprint$eUDPLoss1),]
nrow(sprintSortedEast)

#Verizon
verizon<-mydat[mydat$Provider == "Verizon",]
verizonsorted<-verizon[order(verizon$wUDPLoss1),]
verizonSortedEast<-verizon[order(verizon$eUDPLoss1),]

#T-Mobile
tmobile<-mydat[mydat$Provider == "T-Mobile",]
tmobilesorted<-tmobile[order(tmobile$wUDPLoss1),]
tmobileSortedEast<-tmobile[order(tmobile$eUDPLoss1),]

#AT&T
att<-mydat[mydat$Provider == "AT&T",]
attsorted<-att[order(att$wUDPLoss1),]
attSortedEast<-att[order(att$eUDPLoss1),]
...
ggplot(west, aes(percent)) + 
  ...
  scale_y_continuous(name="West Average UDP Loss 1 (Milliseconds)") +
  ggtitle("West Phone")
...
ggplot(east, aes(percent)) + 
  ...
  scale_x_continuous(name="Percentage (%)", limits=c(0, 100)) +
  scale_y_continuous(name="East Average UDP Loss 1 (Milliseconds)") +
  ggtitle("East Phone")
