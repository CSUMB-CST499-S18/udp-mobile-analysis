...
mydatafiltered$wUDPJit2 = as.character(mydatafiltered$wUDPJit2)
mydatafiltered$eUDPJit2 = as.character(mydatafiltered$eUDPJit2)
mydatafiltered$wUDPJit2 <- gsub("^[a-z].*", "Inf", mydatafiltered$wUDPJit2)
mydatafiltered$eUDPJit2 <- gsub("^[a-z].*", "Inf", mydatafiltered$eUDPJit2)


#convert char to numeric 
mydatafiltered$wUDPJit2 <- as.numeric(as.character(mydatafiltered$wUDPJit2))
mydatafiltered$eUDPJit2 <- as.numeric(as.character(mydatafiltered$eUDPJit2))
...
getavgWest<-function(x,sdata){
  ...
  newdata<-sdata[1:y,]
  sumnewdata<-sum(newdata$wUDPJit2)
  print(sumnewdata)
...
getavgEast<-function(x,sdata){
  ...
  newdata<-sdata[1:y,]
  sumnewdata<-sum(newdata$eUDPJit2)
  avgNewData<-sumnewdata/y
  avgNewData<-round(avgNewData, digits = 3)
  return(avgNewData)
}
...
sprintSorted<-sprint[order(sprint$wUDPJit2),]
sprintSortedEast<-sprint[order(sprint$eUDPJit2),]
nrow(sprintSortedEast)

#Verizon
verizon<-mydat[mydat$Provider == "Verizon",]
verizonSorted<-verizon[order(verizon$wUDPJit2),]
verizonSortedEast<-verizon[order(verizon$eUDPJit2),]

#T-Mobile
tmobile<-mydat[mydat$Provider == "T-Mobile",]
tmobileSorted<-tmobile[order(tmobile$wUDPJit2),]
tmobileSortedEast<-tmobile[order(tmobile$eUDPJit2),]

#AT&T
att<-mydat[mydat$Provider == "AT&T",]
attSorted<-att[order(att$wUDPJit2),]
attSortedEast<-att[order(att$eUDPJit2),]
...
ggplot(west, aes(percent)) + 
  ...
  scale_y_continuous(name="West Average UDP Jitter 2 (Milliseconds)", limits=c(0,15)) +
  ggtitle("West Phone UDP Jitter 2 (Round 12)")
...
ggplot(east, aes(percent)) + 
  ...
  scale_x_continuous(name="Percentage (%)", limits=c(0, 100)) +
  scale_y_continuous(name="East Average UDP Jitter 2 (Milliseconds)", limits=c(0,15)) +
  ggtitle("East Phone UDP Jitter 2 (Round 12)")
