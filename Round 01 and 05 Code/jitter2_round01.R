...
mydatafiltered$wUDP.Jitter2 = as.character(mydatafiltered$wUDP.Jitter2)
mydatafiltered$eUDP.Jitter2 = as.character(mydatafiltered$eUDP.Jitter2)
mydatafiltered$wUDP.Jitter2 <- gsub("^[a-z].*", "Inf", mydatafiltered$wUDP.Jitter2)
mydatafiltered$eUDP.Jitter2 <- gsub("^[a-z].*", "Inf", mydatafiltered$eUDP.Jitter2)


#convert char to numeric 
mydatafiltered$wUDP.Jitter2 <- as.numeric(as.character(mydatafiltered$wUDP.Jitter2))
mydatafiltered$eUDP.Jitter2 <- as.numeric(as.character(mydatafiltered$eUDP.Jitter2))
...
getavgWest<-function(x,sdata){
  ...
  newdata<-sdata[1:y,]
  sumnewdata<-sum(newdata$wUDP.Jitter2)
  print(sumnewdata)
...
getavgEast<-function(x,sdata){
  ...
  newdata<-sdata[1:y,]
  sumnewdata<-sum(newdata$eUDP.Jitter2)
  avgNewData<-sumnewdata/y
  avgNewData<-round(avgNewData, digits = 3)
  return(avgNewData)
}
...
sprintSorted<-sprint[order(sprint$wUDP.Jitter2),]
sprintSortedEast<-sprint[order(sprint$eUDP.Jitter2),]
nrow(sprintSortedEast)

#Verizon
verizon<-mydat[mydat$Provider == "Verizon",]
verizonSorted<-verizon[order(verizon$wUDP.Jitter2),]
verizonSortedEast<-verizon[order(verizon$eUDP.Jitter2),]

#T-Mobile
tmobile<-mydat[mydat$Provider == "T-Mobile",]
tmobileSorted<-tmobile[order(tmobile$wUDP.Jitter2),]
tmobileSortedEast<-tmobile[order(tmobile$eUDP.Jitter2),]

#AT&T
att<-mydat[mydat$Provider == "AT&T",]
attSorted<-att[order(att$wUDP.Jitter2),]
attSortedEast<-att[order(att$eUDP.Jitter2),]
...
ggplot(west, aes(percent)) + 
  ...
  scale_y_continuous(name="West Average UDP Jitter 2 (Milliseconds)") +
  ggtitle("West Phone UDP Jitter 2 (Round 1)")
...
ggplot(east, aes(percent)) + 
  ...
  scale_x_continuous(name="Percentage (%)", limits=c(0, 100)) +
  scale_y_continuous(name="East Average UDP Jitter 2 (Milliseconds)") +
  ggtitle("East Phone UDP Jitter 2 (Round 1)")
